...
class StandUpController {
  ...
  async updateStandUp(req, res) {
    ...
    try {
      ...
      res.status(404).json({ message: 'StandUp not found!', errors: e.message });
    }
    ...
    try {
      ...
      res.status(404).json({ message: 'Error updating standup!', errors: e.message });
    }
  ...
  async completeStandUp(req, res) {
    ...
    try {
      ...
      res.json({ StandUp: { message: 'There was an error creating new StandUp', errorDetails: e } });
    }
  }
  ...
  async subscribeToStandUp(req, res) {
    ...
    try {
      ...
      res.status(500).json({ message: 'Error getting user', errors: e.message });
    }

    if (user.standups.length > 0) {
      if (user.standups.indexOf(standUp.id) > -1) {
        res.status(409).json({ success: false, message: 'User already subscribed', user });
      }
    }
    ...
    try {
      ...
      res.status(404).json({ message: 'Error subscribing user to standup!', errors: e.message });
    }
  ...
  async unsubscribeToStandUp(req, res) {
    ...
    if (user.standups.length > 0) {
      ...
      if (scrumIndex === -1) {
        res.status(409).json({ success: false, message: 'User not subscribed to scrum', user });
      }
    }
