...
const authinstallationStore = {
  storeInstallation: async (installation) => {
    const existsInstall = await SlackInstallation.findById(installation.team.id);
  ...
  fetchInstallation: async (installQuery) => {
    if (installQuery.teamId !== undefined) {
      const fetch = await SlackInstallation.findById(installQuery.teamId)
        .lean();
  ...
  deleteInstallation: async (installQuery) => {
    if (installQuery.team_id !== undefined) {
      await SlackInstallation.findByIdAndDelete(installQuery.team_id);
      return;
    }
